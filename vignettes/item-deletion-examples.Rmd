...
Example from Lai, Kwok, Yoon and Hsiao (2017), using the Center for
Epidemiological Studies Depression (CES-D) Scale. 

We first demonstrate the use of the item deletion indices and relevant
functions by treating each subscale of the CES-D Scale as a separate factor, and
assume that classification decisions will be based on the sum of the subscale
scores. Then, we demonstrate the use of the item deletion indices with the full
four dimensional CES-D Scale.
...
$h^{|1}\ov{\SP}=-0.059$, which suggests a slight improvement in all three 
$\CAI$ $(h < .20)$.
...
deletion scenarios and $\ov{\SR}$ and $\ov{\SE}$ are slightly lower at around 
$0.820$, suggesting that the Depressive Affect subscale is best at not selecting 
the individuals who should not be selected. We see that aggregate SE, aggregate
SR, and aggregate SP  for the full item set are higher than the other item 
subsets, suggesting that the removal of an item will likely not improve 
aggregate CAI. h values in the second table are mostly positive and indicate a 
slight decrease in $\ov{\CAI}$ for each item set, which suggests a drop in 
performance. 
...
In light of the conflicting findings and the small effect size of the potential 
...
The first table illustrates that reemoving item 4 from the Somatic Complaints 
subscale leads to a decreases in aggregate SR, aggregate SE, and aggregate SP, 
suggesting worse performance if this item is dropped.

The deletion of item 4 brings the AI ratio from the already-high $0.998$ to $1$.
Similarly, removing this item brings the already-small difference between the
reference and Efocal groups to $0$ with a very small effect size of around
$0.003$. These findings suggest that removing the fourth item will harm or
lead to a negligible improvement in CAI, therefore the full item set should be
retained.
...
is approximately a small effect size. The adverse impact ratio goes up to $1$
from $0.966$ and $\h^{\rr}\CAI^{|2}=0$ for all indices. The effect size of the
improvement in the discrepancy between the reference and Efocal groups on SR when 
item 2 is deleted is $\Delta^{|2}\h^{\rr}\SR=0.132$,
$\Delta^{|2}\h^{\rr}\SE=0.062$, and $\Delta^{|2}\h^{\rr}\SE=0.086$.
Given these conflicting findings and the fact that this subscale only has two
items, it may be best to retain both items if the subscale will be used on its 
...
correspond to SOM 4, DEP 2, DEP 3,DEP 4, POS 1, INT 1, and INT 2. Removing any 
of these items either decreases $\ov{\CAI}$ or leads to a negligible increase. 
The second table shows negative h values for $\ov{\SR}^{|15}$ and
$\ov{\SP}^{|15}$, suggesting a very small improvement in performance if item 15
(POS 1) is dropped. Similarly, the largest increase in AI ratio is observed when
item 15 is dropped: $AI=0.899$ to $AI^{|15}=0.975$, indicating that this item is
a strong candidate for deletion. The second largest increase is achieved by the
deletion of item 10 (DEP 3), with $AI^{|10}=0.923$. 
...
$\CAI_{Ef}$ becomes smallest if item 15 is deleted as $\h^{\rr}\PS=0.093$ goes 
down to $\h^{\rr}\PS^{|15}=0.022$, $\h^{\rr}\SR=-0.151$ goes up to 
$\h^{\rr}\SR^{|15}=-0.044$, $\h^{\rr}\SE=0.143$ goes down to 
$\h^{\rr}\SE^{|15}=0.026$ and $\h^{\rr}\SP=-0.63$ goes up to 
$\h^{\rr}\SP^{|15}=-0.046$. Note that we judge improvement in the difference 
...
choose between the two items. The output object from item_deletion_h contains 
other tables that may aid the researcher in this process. For instance, the 
outputs of PartInv() may be examined for the two item deletion scenarios:
...
item 15. We can repeat the process with the new item weights:

```{r}
CESD_full_drop15 <- item_deletion_h(
                          cut_z = 16,
                          weights_item = c(rep(1,14), 0, rep(1.33333, 3), 1, 1),
                          weights_latent = c(rep(1,4)),
                          alpha_r = alpha_CESD_r,
                          alpha_f = alpha_CESD_f,
                          psi_r = psi_cesd,
...
Now to illustrate `print_formatted=FALSE`:
