...
```{r CESD, include=TRUE}
lambda_r <- rbind(cbind(lambda_som_r, rep(0, 7), rep(0, 7), rep(0, 7)),
...
lambda_f <- rbind(cbind(lambda_som_f, rep(0, 7), rep(0, 7), rep(0, 7)),
...
psi_r <- c(psi_som_r, psi_dep_r, psi_pos_r, psi_int_r)
psi_f <- c(psi_som_f, psi_dep_f, psi_pos_f, psi_int_f)
alpha_r <- c(alpha_som_r, alpha_dep_r, alpha_pos_r, alpha_int_r)
alpha_f <- c(alpha_som_f, alpha_dep_f, alpha_pos_f, alpha_int_f)

Theta_r <- diag(c(0.45, 0.37, 0.39, 0.40, 0.57, 0.51, 0.37, #SOM
                  0.29, 0.30, 0.41, 0.31, 0.29, 0.27, 0.24, #DEP
                  1.20, 0.81, 0.32, 0.32, #POS
                  0.19, 0.23 #INT
                  ))
Theta_f <- diag(c(0.45, 0.37, 0.39, 0.40, 0.57, 0.51, 0.37, #SOM
                  0.29, 0.13, 0.09, 0.14, 0.29, 0.27, 0.24, #DEP
                  0.72, 0.81, 0.32, 0.32, #POS
                  0.19, 0.08 #INT
                  ))

nu_r <- as.matrix(c(nu_som_r, nu_dep_r, nu_pos_r, nu_int_r))
nu_f <- as.matrix(c(nu_som_f, nu_dep_f, nu_pos_f, nu_int_f))
...
S_f <- diag(sqrt(psi_f))
S_r <- diag(sqrt(psi_r))
psi_f <- S_f %*% corr_CESD %*% S_f
psi_r <- S_r %*% corr_CESD %*% S_r
...
CESD_partinv <- PartInv(
  ...
  plot_contour = FALSE,
  pmix = c(pmix_r, 1 - pmix_r)
)
CESD <- item_deletion_h(
  CESD_partinv, item_which_dim = rep(1:4, c(7, 7, 4, 2)))
  
# equivalently, in the two-group scenario, users can specify the parameters
# by group as below:
# CESD <- item_deletion_h(
#   cut_z = 16, weights_item = c(rep(1, 20)), weights_latent = c(7, 7, 4, 2),
#   alpha_r = alpha_r, alpha_f = alpha_f, psi_r = psi_r, psi_f = psi_f,
#   lambda_r = lambda_r, lambda_f = lambda_f, nu_r = nu_r,  nu_f = nu_f,
#   Theta_r = Theta_r, Theta_f = Theta_f, plot_contour = FALSE, n_dim = 4, 
#   n_i_per_dim = c(7, 7, 4, 2), pmix_ref = pmix_r)
```
...
```{r}
CESD_del1_cutoff <- 
  item_deletion_h(CESD_partinv,
                  item_which_dim = rep(1:4, c(7, 7, 4, 2)),
                  delete_one_cutoff = 16 / 60 * (60 - 3))
CESD_del1_cutoff
```

Let's assume for the sake of illustration that the researcher decides to drop
item 15 ($good$). We can repeat the process with the new item weights, with
the `update_latent_weights = TRUE` argument to adjust the latent weights
to reflect less weight on the dimension with the deleted item:
...
CESD_del1_cutoff <- 
  item_deletion_h(CESD_partinv,
                  item_which_dim = rep(1:4, c(7, 7, 4, 2)),
                  delete_one_cutoff = 16 / 60 * (60 - 3),
                  update_latent_weights = TRUE)
```

```{r}
CESD_drop15_partinv <- CESD_del1_cutoff$delete_one_outputs$`|15`
CESD_drop15 <- item_deletion_h(
  CESD_drop15_partinv,
  item_which_dim = rep(1:4, c(7, 7, 3, 2))
)
CESD_drop15
```
...
```{r}
CESD_drop15$delete_one_outputs$`|10`
```
...
CESD_som_partinv <- PartInv(
  cut_z = 16 / 60 * 7 * 3,
  weights_item = c(rep(1, 7)),
  weights_latent = 1,
  alpha = list(alpha_som_r, alpha_som_f),
  psi = list(psi_som_r, psi_som_f),
  lambda = list(lambda_som_r, lambda_som_f),
  nu = list(nu_som_r, nu_som_f),
  theta = list(Theta_som_r, Theta_som_f),
  pmix = c(pmix_r, 1 - pmix_r),
  plot_contour = FALSE
)
CESD_som <- item_deletion_h(CESD_som_partinv)
CESD_som
```
...
CESD_dep_partinv <- PartInv(
  cut_z = 16 / 60 * 7 * 3,
  weights_item = c(rep(1, 7)),
  weights_latent = 1,
  alpha = list(alpha_dep_r, alpha_dep_f),
  psi = list(psi_dep_r, psi_dep_f),
  lambda = list(lambda_dep_r, lambda_dep_f),
  nu = list(nu_dep_r, nu_dep_f),
  theta = list(Theta_dep_r, Theta_dep_f),
  pmix = c(pmix_r, 1 - pmix_r),
  plot_contour = FALSE
)
CESD_dep <- item_deletion_h(CESD_dep_partinv)
CESD_dep
```
...
CESD_pos_partinv <- PartInv(
  cut_z = 16 / 60 * 12,
  weights_item = c(rep(1, 4)),
  weights_latent = 1,
  alpha = list(alpha_pos_r, alpha_pos_f),
  psi = list(psi_pos_r, psi_pos_f),
  lambda = list(lambda_pos_r, lambda_pos_f),
  nu = list(nu_pos_r, nu_pos_f),
  theta = list(Theta_pos_r, Theta_pos_f),
  pmix = c(pmix_r, 1 - pmix_r),
  plot_contour = FALSE
)
...
CESD_int_partinv <- PartInv(
  cut_z = 16 / 60 * 2 * 3,
  weights_item = c(rep(1, 2)),
  weights_latent = 1,
  alpha = list(alpha_int_r, alpha_int_f),
  psi = list(psi_int_r, psi_int_f),
  lambda = list(lambda_int_r, lambda_int_f),
  nu = list(nu_int_r, nu_int_f),
  theta = list(Theta_int_r, Theta_int_f),
  pmix = c(pmix_r, 1 - pmix_r),
  plot_contour = FALSE
)
CESD_int <- item_deletion_h(CESD_int_partinv)
CESD_int
```
...
We can also specify which specific items we would like to examine using the 
`delete_items`:
...
CESD_pos <- item_deletion_h(CESD_pos_partinv, delete_items = 1:3)
CESD_pos
```
...
```{r }
CESD_pos <- item_deletion_h(CESD_pos_partinv)
print(CESD_pos, full_result = TRUE)
```

We can obtain plots using the `plot` method. Additional optional parameters allow the plots to be customized with colors, group labels, and quadrant labeling (A, B, C, D or TR, FP, TN, FN).

```{r fig.width=7, fig.height=6}
plot(CESD_pos_partinv, 
     custom_colors = c("salmon1", "lightgreen"),
