...
e.g., SE under partial (factorial) invariance (PFI): $\SE^{\par}$, SE for item 
set $I-i_k$ for the reference group: $\SE^{(-k); r}$, and reserve the subscript
of the acronym to indicate that the current operation (e.g., $\Delta h$) is being performed on h 
...
SAI when item $i_k$ is removed (under PFI): e.g.,
$h^{(-k)}(\td{\SE}^{\par})$ for performing the operation on SE under PFI. The resulting h values: $h^{(-k)}_{\td{\SE}^{\par}}$.
...
under PFI. We can denote this as, for SE, $\Delta h^{(-k)}({\SE^{\par}_{r,Ef}})$.
...
PFI, for a given set of items for the reference group. We can 
denote this as: $\Delta h^{(-k)}(\SE^{r}_{\str,\par})$.
...
The first element of the results, `h_overall_sai.par`, is a list containing effect 
sizes showing the change in a selection accuracy index (SAI) under for the full 
item set vs. when one item is deleted, using parameter estimates from the PFI model. 'Overall' refers to the weighting of accuracy indices for the 
focal and reference groups by the group proportions to arrive at a weighted
average for each index. For instance, the overall SR is computed as
\begin{align*}
\td{\SR} &= \dfrac{\text{TP}_{r}\times p_r + \text{TP}_{f}\times (1-p_r)}{\text{TP}_{r}\times p_r + \text{TP}_{f}\times (1-p_r) + \text{FP}_{r}\times p_r + \text{FP}_{f}\times (1-p_r)}
\end{align*}

where $p_r$ denotes the weight for the reference group. 

The overall SE is computed as
\begin{align*}
\td{\SE} &= \dfrac{\text{TP}_{r}\times p_r + \text{TP}_{f}\times (1-p_r)}{\text{TP}_{r}\times p_r + \text{TP}_{f}\times (1-p_r) + \text{FN}_{r}\times p_r + \text{FN}_{f}\times (1-p_r)}
\end{align*}
and overall SP is computed as 
\begin{align*}
\td{\SP} &= \dfrac{\text{TN}_{r}\times p_r + \text{TN}_{f}\times (1-p_r)}{\text{TN}_{r}\times p_r + \text{TN}_{f}\times (1-p_r) + \text{FP}_{r}\times p_r + \text{FP}_{f}\times (1-p_r)}
\end{align*}


[HL]: # (For the last sentence above, could you define the overall index in formula? Are they the weighted averages of true positive/false positive?) - DONE

[HL]: # (Also note: I would use 'PI' or 'PFI' as an abbreviation for partial (factorial) invariance) - DONE
...
\[
\begin{align*}
h^{(-k)}(\td{\SE}^{\par}) &= 2\arcsin(\sqrt{\td{\SE}^{\par}})-2\arcsin(\sqrt{\td{\SE}^{(-k);\par}})\\
&=h^{(-k)}_{\td{\SE}^{\par}}
\end{align*}
\]
\[
\begin{align*}
h^{(-k)}(\td{\SP}^{\par}) &= 2\arcsin(\sqrt{\td{\SP}^{\par}})-2\arcsin(\sqrt{\td{\SP}^{(-k);\par}})\\
&=h^{(-k)}_{\td{\SP}^{\par}}
\end{align*}
\]
```{r}
...
followed the same distribution as the reference group under PFI. When SFI holds, the $h_{R - EF}(\PS)$ values are zero.
...
[HL]: # (Should it be SE instead of PS towards the end of the first line of the equation?) - DONE

\[
\begin{align*}
\Delta h^{(-k)}({\SE^{\par}_{r,Ef}}) &= \text{sign}\left(h_{\SE_{r,Ef}} - h_{\SE^{(-k)}_{r,Ef}}\right)||h_{\SE^{(-k)}_{r,Ef}}|-|h_{\SE_{r,Ef}}||\\
&=\Delta h^{(-k)}({\SE^{\par}_{r,Ef})}
\end{align*}
\]
...
reference and focal groups under SFI vs. PFI for a given set 
...
item 1 brought the effect size of the difference between SFI and PFIe conditions to 0 for all accuracy indices.

`delta_h$h_str_vs_par$ref`, `delta_h$h_str_vs_par$foc`

Here, the comparison is between h values for the full item set vs. h when item i is deleted. These h values correspond to the comparison between accuracy indices under SFI vs. PFIe, for the reference and focal groups. 
