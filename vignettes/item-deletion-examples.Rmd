...
[HL]: # (Also, the `SR_SE_SP` seems too long. Can we call it `sai` for selection accuracy indices? I used that in some papers.) [DONE]
[HL]: # (I think we can directly use the italic h instead of text h. [DONE] 
For indices that you're going to use repeatedly, maybe define a LaTeX macro.) [to-do]

`h_overall_sai_par` is a list containing effect sizes showing the change in
...
$$h_{\text{SR}}^{(-i)} = 2\arcsin(\sqrt{\text{SR}^\text{full}})-2\arcsin(\sqrt{\text{SR}^{(-i)}})$$
```{r}
# HL: Maybe show all four as all of them are discussed below?
knitr::kable(ex1_output$h_overall_sai_par)
```
For the 4-item CES-D scale, the negative $h_{\text{SR}}^{(-1)}$,
$h_{\text{SE}}^{(-1)}$, and $h_{\text{SP}}^{(-1)}$ values 
...
SR for the full item set for a positive $h_{\text{SR}}^{(-2)}$ to be 
...
$$h_{{\text{PS}}^{(\text{full})}_\text{ref_Ef}} = 2\text{arcsin}(\sqrt{\text{PS}_{\text{ref}}^{(\text{full})}})-2\text{arcsin}(\sqrt{\text{PS}_{\text{Efocal}}^{(\text{full})}})$$
$$h_{{\text{PS}}^{(-i)}_\text{ref_Ef}} = 2\text{arcsin}(\sqrt{\text{PS}_{\text{ref}}^{(-i)}})-2\text{arcsin}(\sqrt{\text{PS}_{\text{Efocal}}^{(-i)}})$$
```{r }
knitr::kable(ex1_output$h_R_vs_Ef_par)
```

The 0 values for $h_{{\text{PS}}^{(-1)}_{\text{ref_Ef}}}$,$h_{{\text{SR}}^{(-1)}_\text{ref_Ef}}$,
$h_{{\text{SE}}^{(-1)}_\text{ref_Ef}}$, $h_{{\text{SP}}^{(-1)}_\text{ref_Ef}}$ indicate that 
...
For the full set of items, we see that $h_{{\text{PS}}^{(\text{full})}_\text{Efocal}}=0.203$, suggesting that if 
...
[HL]: # (For the following, should we also do the values for the focal group as well?) [DONE]

**h_str_vs_par_ref** and **h_str_vs_par_focal** compare accuracy indices for the
reference and focal groups under strict vs. partial invariance for a given set 
of items (full, one item deleted).

$$h_{{\text{PS}}^{(\text{full})}_\text{str_par}} = 2\text{arcsin}(\sqrt{\text{PS}_{\text{str}}^{(\text{full})}})-2\text{arcsin}(\sqrt{\text{PS}_{\text{par}}^{(\text{full})}})$$
$$h_{{\text{PS}}^{(-i)}_\text{str_par}} = 2\text{arcsin}(\sqrt{\text{PS}_{\text{str}}^{(-i)}})-2\text{arcsin}(\sqrt{\text{PS}_{\text{par}}^{(-i)}})$$

```{r }
ex1_output$h_str_vs_par_ref
ex1_output$h_str_vs_par_focal
```
$h_{{\text{PS}}^{(\text{full})}_\text{str_par}}=-0.040$ indicates that 
...
**delta_h_str_vs_par_ref**, **delta_h_str_vs_par_focal**
...
**h_overall_sai_par**
$$h_{{\text{SR}}^{(-i)}} = 2\text{arcsin}(\sqrt{\text{SR}^\text{full}})-2\text{arcsin}(\sqrt{\text{SR}^{(-i)}})$$

```{r}
ex2_output$h_overall_sai_par
```
...
there would be an increase in PS for the reference group, leading to a positive $h_{{\text{PS}}^{(-i)}_\text{ref_Ef}}$.
...
```{r}
ex3_output$h_overall_sai_par
```

Looking at **h_overall_sai_par**, we see that dropping item 1 would lead to
...
ex3_output$h_str_vs_par_focal
```

```{r}
ex3_output$delta_h_str_vs_par_ref
ex3_output$delta_h_str_vs_par_focal
```
