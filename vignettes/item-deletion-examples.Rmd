...
We can denote the set of items excluding a specific item 'k' as 
$I-i_k$, e.g., if the third item $i_3$ is excluded, the remaining set of items 
will be denoted by $I-i_3$. If referring to the $h$ or $\Delta h$ associated with
the removal of an item $i_k$, the operation $h(I,I-i_k)$ can be referred to as 
$h^{-i_k}$ and $\Delta h(I,I-i_k)$ as $\Delta h^{-i_k}$. Accuracy indices will be
...
$\td{\SR}$ etc. We can reserve the superscript for non-comparison attributes, e.g.,
SE under partial invariance: $\SE^{\par}$, SE for item set $I-i_k$ for the reference 
group: $\SE^{I-i_k;r}$, and reserve the subscript of the acronym to indicate that 
the current operation (e.g., $\Delta h$) is being performed on h values. For instance,
on Cohen's h values associated with the comparison between SAI under strict vs. partial invariance, for the reference group: $\SE_{\str,\par}^{r}$. We can use commas in 
subscripts of acronyms to indicate the pair of groups/conditions compared, and semicolons
...
$h^{-i_k}(\td{\SE}^{\par})$ for performing the operation on SE under partial 
invariance. The resulting h values: $h^{-i_k}_{\td{\SE}^{\par}}$.
...
under partial invariance. We can denote this as, for SE, $\Delta h^{-i_k}(h_{\SE^{\par}_{r,Ef}})$.
...
denote this as: $\Delta h^{-i_k}(h_{\SE^{r}_{\str,\par}})$.
...
ex1 <- item_deletion_h(weights_item = c(1, 1, 1,  1), 
                              ...
                              also_return_PartInvMulti_outputs = TRUE,
                              pmix_ref = 5/7)
```

`h_overall_sai.par` is a list containing effect sizes showing the change in
...
\[
\begin{align*}
h^{-i_k}(\td{\SR}^{\par}) &= 2\arcsin(\sqrt{\td{\SR}_{I}^{\par}})-2\arcsin(\sqrt{\td{\SR}^{\par}_{I-i_k}})\\
&=h^{-i_k}_{\td{\SR}^{\par}}
\end{align*}
\]
```{r}
...
```
For the 4-item CES-D scale, the negative $h^{-i_1}_{\td{\SR}^{\par}}$,
$h^{-i_1}_{\td{\SE}^{\par}}$, and $h^{-i_1}_{\td{\SP}^{\par}}$ values 
...
SR for the full item set for a positive $h^{-i_2}_{\td{\SR}^{\par}}$ to be 
...
\[
\begin{align*}
h^{r,Ef}(\PS^{\par;I}) &= 2\text{arcsin}(\sqrt{\PS^{\par;I}_r})-2\text{arcsin}(\sqrt{\PS^{\par;I}_{Ef}})\\
&=h_{\PS^{\par;I}_{r,Ef}}
\end{align*}
\]
\[
\begin{align*}
h^{r,Ef}(\PS^{\par;I-i_k}) &= 2\text{arcsin}(\sqrt{\PS^{\par;I-i_k}_r})-2\text{arcsin}(\sqrt{\PS^{\par;I-i_k}_{Ef}})\\
&=h_{\PS^{\par;I-i_k}_{r,Ef}}
\end{align*}
\]
```{r }
...
The 0 values for $h_{\PS^{\par;I-i_1}_{r,Ef}}$,$h_{\SR^{\par;I-i_1}_{r,Ef}}$,
$h_{\SE^{\par;I-i_1}_{r,Ef}}$, $h_{\SP^{\par;I-i_1}_{r,Ef}}$ indicate that 
...
For the full set of items, we see that $h_{\PS^{\par;I}_{r,Ef}}=0.203$, suggesting that if 
...
\[
\begin{align*}
\Delta h^{-i_k}(h_{\SE^{\par}_{r,Ef}}) &= \text{sign}\left(h_{\SE^{I}_{r,Ef}} - h_{\SE^{I-i_k}_{r,Ef}}\right)||h_{\SE^{(I-i_k)}_{r,Ef}}|-|h_{\PS^{I}_{r,Ef}}||\\
&=\Delta h^{-i_k}(h_{\SE^{\par}_{r,Ef})})
\end{align*}
\]
```{r }
...
\[
\begin{align*}
h^{\str,\par}(\PS^{r;I}) &= 2\text{arcsin}(\sqrt{\PS^{r;I}_{\str}})-2\text{arcsin}(\sqrt{\PS^{r;I}_{\par}})\\
&=h_{\PS^{r;I}_{\str,\par}}
\end{align*}
\]
\[
\begin{align*}
h^{\str,\par}(\PS^{r;I-i_k}) &= 2\text{arcsin}(\sqrt{\PS^{r;I-i_k}_{\str}})-2\text{arcsin}(\sqrt{\PS^{r;I-i_k}_{\par}})\\
&=h_{\PS^{r;I-i_k}_{\str,\par}}
\end{align*}
\]
...
$h_{\PS^{r;I}_{\str,\par}}=-0.040$ indicates that 
...
\[
\begin{align*}
\Delta h^{-i_k}(\PS^{r}_{\str,\par})&= \text{sign}\left(h_{\PS^{r;I}_{r;I}} - h_{\PS^{r;I-i_k}_{\str,\par}} \right)||h_{\PS^{r;I-i_k}_{\str,\par}} |-|h_{\PS^{r;I}_{\str,\par}}||\\
\end{align*}
\]
```{r }
