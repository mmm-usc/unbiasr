---
title: "item_deletion_examples12.2"
...
```{r load functions, cache = FALSE, echo=FALSE}
source(here("item_deletion_h.R"))
source(here("helper_item_deletion.R"))
source(here("PartInv_multi.R"))
source(here("helper.R"))
source(here("PartInv.R"))
```

## Example from Lai, Kwok, Yoon and Hsiao (2017) 
Center for Epidemiological Studies Depression  (CES-D) Scale 
Positive Affect Factor
Items: good, hopeful, happy, enjoyed
Biased items: 1
```{r}
...
by group proportions to arrive at a weighted average for each index.
...
indicate that removing the first item led to an improvement in success ratio, 
sensitivity, and specificity as the reduced set of items led to a larger
SR, SE, SP. Removing any of the other items harmed SR, SE, SP (indicated by the
positive h values).
...
selected in the reference group is larger than the proportion selected in the Efocal
group when we use the full set of items. Under the expected distribution for the reference group, the Efocal group would 
...
```
No clear distinction between the biased and non-biased items.
